...
echo "Creating A/B partition layout for ARM64..."

# Create A/B partition disk image (128MB total)
# Partition layout:
# - Boot partition (16MB) - shared kernel/dtb
...
if command -v mcopy >/dev/null 2>&1; then
    if [ -f "${IMAGES_DIR}/Image" ]; then
        if mcopy -i "${BOOT_IMAGE}" "${IMAGES_DIR}/Image" ::Image 2>/dev/null; then
    ...
    if [ -f "${IMAGES_DIR}/qemu-aarch64.dtb" ]; then
        if mcopy -i "${BOOT_IMAGE}" "${IMAGES_DIR}/qemu-aarch64.dtb" ::qemu-aarch64.dtb 2>/dev/null; then
            echo "✓ DTB copied to boot partition"
        else
            echo "⚠ DTB copy failed"
        fi
    fi
else
...
boot_from_part=fatload virtio 0:1 0x40400000 Image; if test "${boot_slot}" = "a"; then setenv bootargs "root=/dev/vdb2 rootwait console=ttyAMA0"; else setenv bootargs "root=/dev/vdb3 rootwait console=ttyAMA0"; fi; booti 0x40400000 - $fdtcontroladdr
EOF
